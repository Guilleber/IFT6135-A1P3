...
class ConvNet(nn.Module):
    ...
    def forward(self, input_batch)
        ...
        preactivation = self.OutLayer(input_batch.contiguous().view(-1))
    ...
    def trainer(self, optimizer):
        def train_step(batch, use_cuda=False):
            ...
            return int(loss.data.cpu().numpy()[0])
        return train_step
    ...
    def validator(self):
        def valid_step(batch):
            ...
            return int(loss.data.cpu().numpy()[0]), acc
        return valid_step
...
def out_dim_pool(in_dim, kernel_size):
    return math.floor((in_dim - (kernel_size - 1) - 1)/kernel_size)