...
class ConvNet(nn.Module):
    def __init__(self, params):
		...
		for i in range(self.params["nb_conv_layers"]):
		    W = out_dim(W, self.params["padding"][i], 1, self.params["kernel_size"][i], self.params["stride"][i])
			H = out_dim(H, self.params["padding"][i], 1, self.params["kernel_size"][i], self.params["stride"][i])
			
		OutLayer = nn.Linear(H*W, 2)


    def forward(self, input_batch)
        for i in range(self.params["nb_conv_layers"]):
            input_batch = Convs[i](input_batch)
        out			
